...
class CategoryDbManager extends DbManager
{
     ...
     */
    public const FIELDS = ["id_category", "name_category", "last_update", "deleted"];

    /**
     * Stores the placeholders for prepared queries.
     */
    public const PLACEHOLDERS = [":idC", ":nameC", ":update"];
    ...
    /**
     * Creates a category in the database from a name given in parameter.
     * @param int $id The id of the category to create.
    ...
    /**
     * Query all category id and last_update fields from database in order to use it to determine which entities to update.
     * @return null|string The json response if found else null.
     */
