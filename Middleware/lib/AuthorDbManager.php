...
class AuthorDbManager extends DbManager
{
     ...
     */
    public const FIELDS = ["id_author", "name_author", "last_update", "deleted"];

    /**
     * Stores the placeholders for prepared queries.
     */
    public const PLACEHOLDERS = [":idA", ":nameA", ":update"];
    ...
    /**
     * Creates an author in the database from a name given in parameter.
     * @param int $id The id of the author to create.
     * @param string $name The name of the author to create.
    ...
    public function queryAllPaginated(int $start, int $end)
    {
        ...
        $statement = sprintf("SELECT %s, %s, %s FROM %s WHERE deleted = 0 LIMIT %s OFFSET %s",
            static::FIELDS[0], static::FIELDS[1], static::FIELDS[2], static::TABLE, $limitPlaceholder, $offsetPlaceholder);
        $req = $this->db->prepare($statement);
    ...
    /**
     * Query all author id and last_update fields from database in order to use it to determine which entities to update.
     * @return null|string The json response if found else null.
     */
